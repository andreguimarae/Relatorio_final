...
def geometry(aircraft: dict) -> dict:
    ...
    cr_w = 2 * S_w / (b_w + b_w * taper_w)
    ct_w = taper_w * cr_w
    yt_w = 0.5*b_w
    xt_w = xr_w + yt_w * np.tan(sweep_w) + 0.25*(cr_w - ct_w)
    zt_w = zr_w + yt_w * np.tan(dihedral_w)
    cm_w = (2/3) * cr_w * (1 + taper_w + taper_w*taper_w)/(1+taper_w)
    ym_w = (b_w/6) * (1+2*taper_w)/(1+taper_w)
    xm_w = xr_w + ym_w * np.tan(sweep_w) + 0.25*(cr_w - cm_w)
    ...
    cr_h = 2 * S_h / (b_h + b_h * taper_h)
    ct_h = taper_h * cr_h
    cm_h = (2/3) * cr_h * (1 + taper_h + taper_h*taper_h)/(1+taper_h)
    xm_h = xm_w + L_h + 0.25*(cm_w - cm_h)
    ym_h = (b_h/6) * (1+2*taper_h)/(1+taper_h)
    zm_h = zr_h + ym_h * np.tan(dihedral_h)
    xr_h = xm_h - ym_h * np.tan(sweep_h) - 0.25*(cr_h - cm_h)
    yt_h = 0.5*b_h
    xt_h = xr_h + yt_h * np.tan(sweep_h) + 0.25*(cr_h - ct_h)
    ...
    cr_v = 2 * S_v / (b_v + b_v * taper_v)
    ct_v = taper_v * cr_v
    cm_v = (2/3) * cr_v * (1 + taper_v + taper_v*taper_v)/(1+taper_v)
    xm_v = xm_w + L_v + 0.25*(cm_w - cm_v)
    zm_v = zr_v + (b_v/3)*(1+2*taper_v)/(1+taper_v)
    xr_v = xm_v - (zm_v - zr_v) * np.tan(sweep_v) - 0.25*(cr_v - cm_v)
    zt_v = zr_v + b_v
    xt_v = xr_v + (zt_v - zr_v) * np.tan(sweep_v) + 0.25*(cr_v - ct_v)

    # declaring output dict
    wing = {}
    EH = {}
    EV = {}
    ...
    dimensions['wing']['cr'] = cr_w
    dimensions['wing']['ct'] = ct_w
    dimensions['wing']['xt'] = xt_w
    dimensions['wing']['yt'] = yt_w
    dimensions['wing']['zt'] = zt_w
    dimensions['wing']['cm'] = cm_w
    dimensions['wing']['xm'] = xm_w
    dimensions['wing']['ym'] = ym_w
    dimensions['wing']['zm'] = zm_w

    dimensions['EH']['S'] = S_h
    dimensions['EH']['L'] = L_h
    dimensions['EH']['b'] = b_h
    dimensions['EH']['cr'] = cr_h
    dimensions['EH']['ct'] = ct_h
    dimensions['EH']['xr'] = xr_h
    dimensions['EH']['xt'] = xt_h
    dimensions['EH']['yt'] = yt_h
    dimensions['EH']['zt'] = zt_h
    dimensions['EH']['cm'] = cm_h
    dimensions['EH']['xm'] = xm_h
    dimensions['EH']['ym'] = ym_h
    dimensions['EH']['zm'] = zm_h
    

    dimensions['EV']['S'] = S_v
    dimensions['EV']['L'] = L_v
    dimensions['EV']['b'] = b_v
    dimensions['EV']['cr'] = cr_v
    dimensions['EV']['ct'] = ct_v
    dimensions['EV']['xr'] = xr_v
    dimensions['EV']['xt'] = xt_v
    dimensions['EV']['zt'] = zt_v
    dimensions['EV']['cm'] = cm_v
    dimensions['EV']['xm'] = xm_v
    dimensions['EV']['zm'] = zm_v
